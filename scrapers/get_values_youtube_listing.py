...
def get_links_filtered(
    value_result: list, value_name: str, value_duration: int
) -> dict:
    value_return = []
    live = "Live".lower()
    for element in (
        ...
        if abs(value_duration - i.get("duration")) <= 3
        and (
            all(live in i.lower() for i in [value_name, i.get("title")])
        ...
        element.update(
            {
                "sequence": _get_similar_strings(
                    _replace_unneccessary_input(value_name), element.get("title")
                ),
...
def get_links_search(value_string: str) -> list:
    results_parsed = []
    results = YoutubeSearch(_replace_unneccessary_input(value_string), max_results=4)
